...
function formatTemplate(template, metadata, url) {
  ...
  while (result !== previousResult) {
    ...
    result = result.replace(/{([^{}]+?)}/g, (match, key) => {
      ...
      if (format && dateFields.includes(variable) && (0, import_moment.default)(value).isValid()) {
        return (0, import_moment.default)(value).format(format);
      }
      return String(value);
    });
  }
...
var LinkFormatterSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  displayGeneralSettings(containerEl) {
    new import_obsidian3.Setting(containerEl).setName("Auto-linking").setDesc("Automatically format links when pasting URLs, except when overriden.").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoLink).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.autoLink = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian3.Setting(containerEl).setName("Failure mode").setDesc('What to do when a link fails to fetch. "Revert" pastes the original URL. "Alert" shows [Failed to fetch title](url).').addDropdown((dropdown) => dropdown.addOption(FailureMode.Revert, "Revert to plain URL").addOption(FailureMode.Alert, "Show failure message").setValue(this.plugin.settings.failureMode).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.failureMode = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian3.Setting(containerEl).setName("Fetch timeout").setDesc("Maximum time (in seconds) to wait for page metadata. (3-60s)").addText((text) => {
    ...
    this.plugin.settings.titleReplacements.forEach((replacement, index) => {
      const setting = new import_obsidian3.Setting(containerEl).setClass("smart-link-formatter-replacement-setting");
    ...
    new import_obsidian3.Setting(containerEl).addButton((button) => button.setButtonText("Add replacement rule").setCta().onClick(() => __async(this, null, function* () {
  ...
  displayClientSettings(containerEl) {
    for (const client of CLIENTS) {
      const setting = new import_obsidian3.Setting(containerEl).setName(`${client.displayName} link format`).setClass("smart-link-formatter-tall-textarea-setting");
...
var SmartLinkFormatterPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
  ...
  handleFormat(clipboardText, editor) {
    return __async(this, null, function* () {
      ...
      const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Fetch timeout")), this.settings.timeoutSeconds * 1e3));
      let didReplace = false;
      try {
        const client = CLIENTS.find((client2) => client2.matches(clipboardText));
        if (client) {
        ...
        new import_obsidian4.Notice("Failed to format link");
  ...
  replacePlaceholder(placeholder, newText, editor) {
    ...
    try {
      ...
      new import_obsidian4.Notice("Error updating link in file.");
      didReplace = false;
    }
  ...
  formatLinkAtCursor(editor) {
    return __async(this, null, function* () {
      ...
      if (!extraction) {
        new import_obsidian4.Notice("No URL found at cursor position");
        return;
      }
      const { url, start, end } = extraction;
      if (this.isBlacklisted(url)) {
        new import_obsidian4.Notice("URL is in blacklisted domains");
        return;
      }
